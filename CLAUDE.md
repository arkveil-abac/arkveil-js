...
`checkPermission` never throws to the caller: network errors, timeouts, and non-OK responses are logged and return `{ granted: false }`. The data methods follow the same contract: `buildReadCondition`/`buildTouchCondition` fall back to `"FALSE"` and `buildWriteChecks` to `SELECT FALSE` in every field the operation has, all with `mode: "UNAVAILABLE"` — a degraded Arkveil never widens access. The deliberate exceptions **throw**, because they are programming/configuration errors rather than runtime conditions: a malformed dataset code (not three lowercase identifier segments), an unknown `operation`, and `ids` that contradict the operation (sent with `CREATE`, missing for `UPDATE`/`DELETE`). `fetchWithRetry` (core) retries `429`/`5xx`/network failures with exponential backoff + jitter; 4xx (except 429) are not retried. Preserve this fail-closed contract when touching the request path.

### Data conditions (row-level security)

`buildReadCondition`/`buildWriteChecks`/`buildTouchCondition` live on the core `Arkveil` class (inherited by `node`, provided by `nest`). Data policies are READ / TOUCH (pre-state, `UPDATE`+`DELETE`) / RESULT (post-state, `CREATE`+`UPDATE`); there is no `WRITE` type and no `invariantSql`. Wire-contract rules encoded there: dataset codes are normalized (trim + lowercase) client-side so cache keys/logs agree with server matching; `operation` is required on a write check and `ids` are required non-empty for `UPDATE`/`DELETE` and absent for `CREATE`; `ids` are always strings on the wire; an empty target set is a client-side no-op (no request, `mode: "NO_OP"`); response parsing ignores unknown fields (additive contract) but a field the operation requires that is **missing** — or an `{{ids}}` template where ids should have been inlined — denies with `reason: "CONTRACT_VIOLATION"` rather than proceeding unchecked; non-`"NORMAL"` `mode` is honored but logged as degraded; `reason: "METADATA_MISSING"` (dataset not registered) is logged distinctly from a policy deny; the `{{ids}}` placeholder appears on a `CREATE` `resultSql` only and is filled via `resolveCreateResultSql` (which wraps the exported `substituteIds`). Bulk mutations by predicate use `conditions/touch`: compose `touchCondition` into the WHERE clause; for a bulk `UPDATE`, add `RETURNING <pk>` and run **only** `resultSql` for the returned ids — never `touchSql` post-hoc. There is **no projection endpoint** — do not stub one against a guessed contract.
